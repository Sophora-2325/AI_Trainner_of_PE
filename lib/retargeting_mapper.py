"""Human biomechanical angle → ROBOTIS OP3 motor position retargeting.

Consumes the output of GeometricIKSolver.solve() — a dict of human joint angles
in degrees — and produces a dict of 20 OP3 motor target positions in radians.
...
class HumanAngleToOP3Mapper:
    """Convert human biomechanical joint angles to ROBOTIS OP3 motor positions."""
    ...
    def __init__(self, motor_limits: Optional[dict[str, tuple[float, float]]] = None):
        ...
        Args:
            motor_limits: {motor_name: (min_rad, max_rad)} from Webots motor devices.
                          If None, uses DEFAULT_MOTOR_LIMITS_RAD from joint_config_op3.
        """
        from .joint_config_op3 import DEFAULT_MOTOR_LIMITS_RAD
        self._limits = motor_limits or DEFAULT_MOTOR_LIMITS_RAD

    # ---- Public API ----

    def map_frame(self, human_angles: dict[str, float]) -> dict[str, float]:
        """Convert a single frame of human angles to OP3 motor positions.
        ...
        # 2. Map each human angle to its OP3 motor
        for angle_name, motor_name in HUMAN_TO_OP3_MAP.items():
        ...
        for name in BALANCE_MOTORS:
            if name not in HUMAN_TO_OP3_MAP.values():
                positions[name] = 0.0
    ...
    def map_sequence(
        ...
        """Convert a sequence of human angle frames to OP3 motor keyframe sequences.
    ...
    def _f_knee(human_deg: float) -> float:
        ...
        return (deviation / 120.0) * g["op3_max_bend_rad"]
    ...
    def _f_hip(human_deg: float) -> float:
        ...
        return (deviation / 135.0) * g["op3_max_flex_rad"]
    ...
    def _f_ankle(human_deg: float) -> float:
        ...
        return (deviation / 20.0) * g["op3_max_dorsi_rad"]
